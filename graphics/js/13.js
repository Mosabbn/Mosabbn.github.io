...
function createScene() {
    //starsPerSide = 11;
    ringRadius = 10; 
    root = starburstsOnSphereA( 400, ringRadius, 100, 1);
    scene.add(root);
}

function starburstsOnSphereA(obj, n, t) {
    let root = new THREE.Object3D();
    let angleStep = 2 * Math.PI / n;
  	for (let i = 0, a = 0; i < n; i++, a += angleStep) {
    	let mesh = starburstA(maxRays, maxRad);
    	let p = getRandomPointOnSphere(ringRadius);
...
function update() {
	...
	let nbrRadius = controls.nbrRadius;
	let maxRays = controls.maxRays;
	if (root)
	    scene.remove(root);
	//let  StarburstA = createStarburstA(maxRays, maxRad);
	root = starburstsOnSphereA(nbrBursts, ringRadius, maxRays, burstRadius);
	scene.add(root);
}
