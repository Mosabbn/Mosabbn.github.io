...
let root = null, Radius;


// Dodecahedron
function createSceneA() {
    Radius = 10;
    root = starburstsOnDodecahedron(400, Radius, 100, 1)
    scene.add(root);
}

function starburstsOnDodecahedron(nbrBursts, Radius, maxRays, maxRad) {
    let root = new THREE.Object3D();
    for (let i = 0; i < nbrBursts; i++) {
        let mesh = starburstA(maxRays, maxRad);
        let p = getRandomPointOnDodecahedron(Radius);
        mesh.position.set(p.x);
        root.add(mesh);
    }
    return root;
}
...
function starburstA(maxRays, maxRad) {
    ...
    for (let i = 0; i < nbrRays; i++) {
        ...
        let dest = getRandomPointOnDodecahedron(r);
...
function update() {
    ...
    if (root)
        scene.remove(root);
    root = starburstsOnDodecahedron(nbrBursts, Radius, maxRays, burstRadius);
    scene.add(root);
}
